...
writeJson(path.join(OUT, "universe.json"), {
  ...
  // shows Lab 3.10: the uat window with its checkbox column, and the promotion
  // carrying US-057 alone drawn on the uat to preprod arrow. The node is the uat
  // one of the level3 diagram, which is laid out differently from the one above,
  // and the row is the checkbox of US-057, the second of the two waiting there,
  // whose title wraps onto two lines and pushes it below the first.
  promotionBranch: "promotion/uat/preprod/2026-09-24-0930",
  promotionNode: "1073,389",
  promotionRows: "512,453",
...
// runUiTest.ts in the extension). It is the state Lab 3.10 walks: two User
// Stories waiting in uat, one of them approved, and the promotion carrying that
// one alone to preprod. Kept apart from the fixture above so every other
// screenshot of the course still shows a project with no promotion in flight.
const PROMOTION_BRANCH_NAME = "promotion/uat/preprod/2026-09-24-0930";
const promotionStories = ["US-057", "US-058"].map((id, i) => {
  const story = u.userStories.find((s) => s.id === id);
  return {
    story,
    number: 61 + i,
    // Merged into integration the day before the promotion window closes, and
    // promoted into uat with it
    merged: {
      id: 61 + i,
      number: 61 + i,
      title: `${story.id} ${story.title}`,
      ...
      authorLabel: person(story.author).name,
      state: "merged",
      mergeDate: iso(22 + i, 11),
      createdAt: iso(21 + i, 9),
      webUrl: `${WEB}/pull/${61 + i}`,
      jobsStatus: "success",
      relatedTickets: ticketsOf(story)
    }
  };
});
const carried = promotionStories[0];

writeJson(path.join(OUT, "git-provider-mock-promotion.json"), {
  _comment:
    "Overlay applied on top of git-provider-mock.json when SFDX_HARDIS_DOC_SCREENSHOTS_PROMOTION is set: the uat window of Lab 3.10, with the two User Stories waiting there, and the promotion carrying the approved one to preprod. Generated by scripts/build/mocks.mjs in the sfdx-hardis-training repository: do not edit by hand.",
  addOpenPullRequests: [
    {
      id: 63,
      number: 63,
      ...
      description: [
        `Promotion branch \`${PROMOTION_BRANCH_NAME}\` carrying 1 User Story approved in \`uat\`, cherry-picked for \`preprod\`.`,
        "",
        "```yaml",
        `promotionPullRequests: [${carried.number}]`,
        "```",
        "",
        ...
        "|---|---|---|---|---|",
        `| #${carried.number} | ${carried.merged.title} | ${carried.merged.authorLabel} | \`${carried.story.branch}\` | \`7c41ab9\` |`,
        "",
        `Tickets: ${carried.story.id}`,
        ""
      ].join("\n"),
      ...
      jobs: [
        {
          ...
          status: "success",
          webUrl: `${WEB}/actions/runs/4263`,
          updatedAt: iso(24, 9)
        }
      ],
      jobsStatus: "success",
      relatedTickets: ticketsOf(carried.story)
    }
  ],
  mergedPullRequestsByBranch: {
    // Newest first, the way the branch window lists them: US-058 is waiting for
    // its sign-off, US-057 is the one the promotion carries
    uat: [promotionStories[1].merged, promotionStories[0].merged]
  }
});
